...
class DenseBlock(nn.Module):
    ...
    def __init__(self, in_features, out_features, norm=None, activation=None):
        ...
        if activation is None:
            ...
            nn.init.xavier_uniform_(self.linear.weight, gain=nn.init.calculate_gain('sigmoid'))
            nn.init.zeros_(self.linear.bias)
        elif activation == 'lrelu':
...
class CNNAndFcsNet(nn.Module):
    ...
    def __init__(self,
        ...
        self._norm = norm
        self._activation = activation
        self._out_put_activation = out_put_activation
    ...
    def build(self):
        ...
        dense_layers.append(
            DenseBlock(channels, self._fc_layers[-1],activation=self._out_put_activation))
...
class Conv3DInceptionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        if self._residual:
            yy = [x]
        return torch.cat(yy + [self._1x1conv(x), self._3x3conv(self._1x1conv_a(x)), 
                               self._5x5_via_3x3conv_b(self._5x5_via_3x3conv_a(self._1x1conv_b(x)))], 1)
...
class SpatialSoftmax3D(torch.nn.Module):
    ...
    def __init__(self, depth, height, width, channel):
        ...
        self.output_channel = self.channel * 3
