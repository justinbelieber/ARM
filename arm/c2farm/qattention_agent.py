...
class QAttentionAgent(Agent):
    ...
    def update(self, step: int, replay_sample: dict) -> dict:
        ...
        action_rot_grip = replay_sample['rot_grip_action_indicies'][:, -1].long()   # [b,4]
        # R = r
        reward = replay_sample['reward'] * 0.01  # [b,]
        ...
        if self._layer > 0:
            cp = replay_sample['attention_coordinate_layer_%d' % (
                    self._layer - 1)][:, -1]
            cp_tp1 = replay_sample['attention_coordinate_layer_%d_tp1' % (
                                 self._layer - 1)][:, -1]
            bounds = torch.cat(
                [cp - self._bounds_offset, cp + self._bounds_offset], dim=1)
            bounds_tp1 = torch.cat(
                [cp_tp1 - self._bounds_offset, cp_tp1 + self._bounds_offset],dim=1)

        proprio = proprio_tp1 = None
        if self._include_low_dim_state:
            proprio = stack_on_channel(replay_sample['low_dim_state']) # [b,11]
            proprio_tp1 = stack_on_channel(replay_sample['low_dim_state_tp1']) # [b,11]
        ...
        with torch.no_grad():
            ...
            # q = Q_target(S',a)    using double DQN 
